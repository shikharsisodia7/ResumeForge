...
### ISSUE-08 (out of scope, pre-existing, unrelated to formatting)

Two `e2e/auth-and-landing.spec.ts` tests ("visiting the dashboard/editor while signed out redirects to Auth0 login") fail in this environment: by the time Playwright's assertion runs, the browser has already followed a second redirect hop from the app's own `/auth/login` to Auth0's hosted universal-login URL, so the URL no longer matches `/\/auth\/login/`. Confirmed via `git diff` that no file in the auth path (`src/proxy.ts`, `src/lib/auth0.ts`, `src/lib/auth/*`) was touched by this pass — this is pre-existing behavior unrelated to resume formatting, not a regression introduced here. Left unfixed to stay in scope; flagged for a separate pass.
